...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) =>
      // cache each asset independently — if one fails (404, typo, missing file),
      // the rest still get cached instead of the whole install silently failing
